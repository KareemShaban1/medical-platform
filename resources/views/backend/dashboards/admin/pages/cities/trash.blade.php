    ...
    <div class="row">
        <div class="col-12">
            <div class="page-title-box">
                ...
                <h4 class="page-title">{{ __('Cities Trash') }}</h4>
            </div>
        </div>
    </div>
...
@push('scripts')
<script>
    let trashTable = $('#cities-table').DataTable({
        processing: true,
        serverSide: true,
        ajax: '{{ route("admin.cities.trash.data") }}',
        columns: [{
                data: 'id',
                name: 'id'
            },
   ...
   function restore(id) {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: '{{ route("admin.cities.restore", ":id") }}'.replace(':id', id),
                    method: 'POST',
                    headers: {
    ...
    function forceDelete(id) {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: '{{ route("admin.cities.force-delete", ":id") }}'.replace(':id', id),
                    method: 'DELETE',
                    headers: {
                        'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                    },
