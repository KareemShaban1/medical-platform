            ...
            <div class="col-12">
                <div class="card shadow-sm">
                    <div class="card-header bg-primary text-white">
                        <h5 class="mb-0"><i class="fas fa-users"></i> {{ __('All Clinic Users') }}</h5>
                    </div>
                    <div class="card-body">
                        <div class="table-responsive">
                            <table id="clinic-users-table" class="table table-bordered table-striped table-hover">
    ...
    <script>
        $(document).ready(function () {
            $('#clinic-users-table').DataTable({
                processing: true,
                serverSide: true,
                ajax: '{{ route('admin.users-management.clinic-users.data') }}',
                columns: [
                    { data: 'id', name: 'id' },
                    { data: 'name', name: 'name' },
        ...
        function deleteClinicUser(id) {
            Swal.fire({
                title: '{{ __('Are you sure?') }}',
                text: '{{ __('You are about to move this clinic user to trash!') }}',
                icon: 'warning',
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        url: '{{ route('admin.users-management.clinic-users.destroy', ':id') }}'.replace(':id', id),
                        type: 'DELETE',
                        data: {
                            _token: '{{ csrf_token() }}'
                        },
                        success: function (response) {
                            if (response.status === 'success') {
                                ...
                                $('#clinic-users-table').DataTable().ajax.reload();
                            } else {
                                Swal.fire(
                                    '{{ __('Error!') }}',
                                    response.message,
