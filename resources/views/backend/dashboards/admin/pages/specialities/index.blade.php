		...
		<div class="col-12">
			<div class="page-title-box">
				<div class="page-title-right">
					@hasPermission('create speciality')
...
$('#itemForm').on('submit', function(e) {
	e.preventDefault();
	let id = $('#itemId').val();
	let url = id ? '{{ route("admin.specialities.update", ":id") }}'.replace(':id', id) :
		'{{ route("admin.specialities.store") }}';
...
window.editItem = function(id) {
	$.get('{{ route("admin.specialities.show", ":id") }}'.replace(':id', id), function(item) {
		...
		$('#itemForm').attr('action',
			'{{ route("admin.specialities.update", ":id") }}'
			.replace(':id', id));
...
window.deleteItem = function(id) {
	Swal.fire({
		...
		if (res.isConfirmed) {
			$.ajax({
				url: '{{ route("admin.specialities.destroy", ":id") }}'
					.replace(':id', id),
				method: 'DELETE',
				data: {
