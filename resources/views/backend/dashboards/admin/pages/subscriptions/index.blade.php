            ...
            <div class="card">
                <div class="card-body">
                    <div class="row g-3 mb-3 align-items-end">
                        ...
                        <div class="col-md-3">
                            <label class="form-label">{{ __('Filter by Plan Type') }}</label>
                        ...
                        <div class="col-md-3">
                            <label class="form-label">{{ __('Filter by Entity Type') }}</label>
                            <select id="entity-type-filter" class="form-select form-select-sm">
                                <option value="all">{{ __('All Entities') }}</option>
                                <option value="App\Models\Clinic">{{ __('Clinics') }}</option>
                                <option value="App\Models\ClinicUser">{{ __('Doctors') }}</option>
                                <option value="App\Models\Supplier">{{ __('Suppliers') }}</option>
                            </select>
                        </div>
                    </div>
                    <div class="table-responsive">
                        <table id="subscriptions-table" class="table table-hover dt-responsive nowrap w-100">
                            <thead>
                                <tr>
                                    <th>{{ __('ID') }}</th>
                                    <th>{{ __('Entity') }}</th>
...
$(document).ready(function() {
    const table = $('#subscriptions-table').DataTable({
        ...
        ajax: {
            ...
            data: function(d) {
                ...
                d.plan_type = $('#plan-type-filter').val() || 'all';
                d.entity_type = $('#entity-type-filter').val() || 'all';
            }
        },
        order: [[0, 'desc']],
        dom: '<"row align-items-center mb-3"<"col-md-6"l><"col-md-6 text-md-end"B>>rt<"row mt-3"<"col-md-5"i><"col-md-7"p>>',
        buttons: [
            { extend: 'excel', className: 'btn btn-sm btn-light', text: '<i class="mdi mdi-file-excel"></i> {{ __('Export Excel') }}' },
            { extend: 'print', className: 'btn btn-sm btn-light', text: '<i class="mdi mdi-printer"></i> {{ __('Print') }}' }
        ],
        columns: [
            { data: 'id', name: 'id' },
    ...
    window.createSubscription = function() {
        $.get('{{ route('admin.subscriptions.create') }}', function(resp) {
            if (resp.success && resp.html) {
                $('#subscription-modal').html(resp.html).modal('show');
            }
        });
    };
    ...
    window.extendSubscription = function(id) {
        Swal.fire({
            ...
            preConfirm: () => {
                ...
                if (!days || days < 1) {
                    Swal.showValidationMessage('{{ __('Please enter a valid number of days') }}');
                }
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: '{{ route('admin.subscriptions.extend', ['id' => '__ID__']) }}'.replace('__ID__', id),
                    type: 'POST',
                    ...
                    success: function(resp) {
                        Swal.fire({
                            title: '{{ __('Success!') }}',
                            text: resp.message || '{{ __('Subscription extended successfully') }}',
                            icon: 'success',
                            confirmButtonColor: '#079184',
                        });
                    ...
                    error: function(xhr) {
                        const error = xhr.responseJSON?.message || '{{ __('Failed to extend subscription') }}';
                        Swal.fire({
                            title: '{{ __('Error') }}',
                            text: error,
                            icon: 'error',
    ...
    window.deleteSubscription = function(id) {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: '{{ route('admin.subscriptions.destroy', ['id' => '__ID__']) }}'.replace('__ID__', id),
                    type: 'DELETE',
                    data: { _token: '{{ csrf_token() }}' },
                    success: function(resp) {
                        Swal.fire({
                            title: '{{ __('Deleted!') }}',
                            text: resp.message || '{{ __('Subscription deleted successfully') }}',
                            icon: 'success',
                            confirmButtonColor: '#079184',
                        });
                        table.ajax.reload(null, false);
                    ...
                    error: function(xhr) {
                        const error = xhr.responseJSON?.message || '{{ __('Failed to delete subscription') }}';
                        Swal.fire({
                            title: '{{ __('Error') }}',
                            text: error,
                            icon: 'error',
