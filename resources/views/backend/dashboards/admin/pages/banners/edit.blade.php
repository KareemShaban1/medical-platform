			...
			<div class="col-lg-8">
				...
				<div class="card">
					<div class="card-body">
						...
						<!-- Text Position -->
						<div class="mb-3">
							<label class="form-label">{{ __('Text Position') }}</label>
							<select name="text_position" id="text_position" class="form-control">
								<option value="top-left" {{ old('text_position', $banner->text_position ?? 'center') == 'top-left' ? 'selected' : '' }}>{{ __('Top Left') }}</option>
								<option value="top-center" {{ old('text_position', $banner->text_position ?? 'center') == 'top-center' ? 'selected' : '' }}>{{ __('Top Center') }}</option>
								<option value="top-right" {{ old('text_position', $banner->text_position ?? 'center') == 'top-right' ? 'selected' : '' }}>{{ __('Top Right') }}</option>
								<option value="center-left" {{ old('text_position', $banner->text_position ?? 'center') == 'center-left' ? 'selected' : '' }}>{{ __('Center Left') }}</option>
								<option value="center" {{ old('text_position', $banner->text_position ?? 'center') == 'center' ? 'selected' : '' }}>{{ __('Center') }}</option>
								<option value="center-right" {{ old('text_position', $banner->text_position ?? 'center') == 'center-right' ? 'selected' : '' }}>{{ __('Center Right') }}</option>
								<option value="bottom-left" {{ old('text_position', $banner->text_position ?? 'center') == 'bottom-left' ? 'selected' : '' }}>{{ __('Bottom Left') }}</option>
								<option value="bottom-center" {{ old('text_position', $banner->text_position ?? 'center') == 'bottom-center' ? 'selected' : '' }}>{{ __('Bottom Center') }}</option>
								<option value="bottom-right" {{ old('text_position', $banner->text_position ?? 'center') == 'bottom-right' ? 'selected' : '' }}>{{ __('Bottom Right') }}</option>
								<option value="custom" {{ old('text_position', $banner->text_position ?? 'center') == 'custom' ? 'selected' : '' }}>{{ __('Custom') }}</option>
							</select>
						</div>

						<!-- Custom Text Position -->
						<div id="custom-text-position" style="display: {{ old('text_position', $banner->text_position ?? 'center') == 'custom' ? 'block' : 'none' }};">
							<div class="row mb-2">
								<div class="col-6">
									<label class="form-label">{{ __('Top') }}</label>
									<input type="text" name="text_position_custom[top]" class="form-control" value="{{ old('text_position_custom.top', $banner->text_position_custom['top'] ?? '') }}" placeholder="10%">
								</div>
								<div class="col-6">
									<label class="form-label">{{ __('Left') }}</label>
									<input type="text" name="text_position_custom[left]" class="form-control" value="{{ old('text_position_custom.left', $banner->text_position_custom['left'] ?? '') }}" placeholder="20%">
								</div>
							</div>
						</div>
						...
						<!-- Button Position -->
						<div class="mb-3">
							<label class="form-label">{{ __('Button Position') }}</label>
							<select name="button_position" id="button_position" class="form-control">
								<option value="below-text" {{ old('button_position', $banner->button_position ?? 'below-text') == 'below-text' ? 'selected' : '' }}>{{ __('Below Text') }}</option>
								<option value="above-text" {{ old('button_position', $banner->button_position ?? 'below-text') == 'above-text' ? 'selected' : '' }}>{{ __('Above Text') }}</option>
								<option value="left-of-text" {{ old('button_position', $banner->button_position ?? 'below-text') == 'left-of-text' ? 'selected' : '' }}>{{ __('Left of Text') }}</option>
								<option value="right-of-text" {{ old('button_position', $banner->button_position ?? 'below-text') == 'right-of-text' ? 'selected' : '' }}>{{ __('Right of Text') }}</option>
								<option value="custom" {{ old('button_position', $banner->button_position ?? 'below-text') == 'custom' ? 'selected' : '' }}>{{ __('Custom') }}</option>
							</select>
						</div>

						<!-- Custom Button Position -->
						<div id="custom-button-position" style="display: {{ old('button_position', $banner->button_position ?? 'below-text') == 'custom' ? 'block' : 'none' }};">
							<div class="row mb-2">
								<div class="col-6">
									<label class="form-label">{{ __('Top') }}</label>
									<input type="text" name="button_position_custom[top]" class="form-control" value="{{ old('button_position_custom.top', $banner->button_position_custom['top'] ?? '') }}" placeholder="80%">
								</div>
								<div class="col-6">
									<label class="form-label">{{ __('Left') }}</label>
									<input type="text" name="button_position_custom[left]" class="form-control" value="{{ old('button_position_custom.left', $banner->button_position_custom['left'] ?? '') }}" placeholder="50%">
								</div>
							</div>
						</div>
						...
						<div class="mb-3">
							...
							<div class="input-group">
								<input type="color" name="text_color" class="form-control form-control-color" value="{{ old('text_color', $banner->text_color ?? '#ffffff') }}">
								<input type="text" name="text_color" class="form-control" value="{{ old('text_color', $banner->text_color ?? '#ffffff') }}" pattern="^#([A-Fa-f0-9]{6}|[A-Fa-f0-9]{3})$">
							</div>
						</div>

						<div class="mb-3">
							<label class="form-label">{{ __('Text Background Color') }} ({{ __('Optional') }})</label>
							<div class="input-group">
								<input type="color" name="text_background_color" class="form-control form-control-color" value="{{ old('text_background_color', $banner->text_background_color ?? '') }}">
								<input type="text" name="text_background_color" class="form-control" value="{{ old('text_background_color', $banner->text_background_color ?? '') }}" pattern="^#([A-Fa-f0-9]{6}|[A-Fa-f0-9]{3})$">
							</div>
						</div>

						<div class="mb-3">
							<label class="form-label">{{ __('Text Background Opacity') }} (0-100)</label>
							<input type="range" name="text_background_opacity" class="form-range" min="0" max="100" value="{{ old('text_background_opacity', $banner->text_background_opacity ?? 0) }}" oninput="this.nextElementSibling.value = this.value">
						...
						<div class="mb-3">
							...
							<select name="text_alignment" class="form-control">
								<option value="left" {{ old('text_alignment', $banner->text_alignment ?? 'left') == 'left' ? 'selected' : '' }}>{{ __('Left') }}</option>
								<option value="center" {{ old('text_alignment', $banner->text_alignment ?? 'left') == 'center' ? 'selected' : '' }}>{{ __('Center') }}</option>
								<option value="right" {{ old('text_alignment', $banner->text_alignment ?? 'left') == 'right' ? 'selected' : '' }}>{{ __('Right') }}</option>
							</select>
						</div>

						<!-- Button Styling -->
						<div class="mb-3">
							<label class="form-label">{{ __('Button Background Color') }}</label>
							<div class="input-group">
								<input type="color" name="button_color" class="form-control form-control-color" value="{{ old('button_color', $banner->button_color ?? '#007bff') }}">
								<input type="text" name="button_color" class="form-control" value="{{ old('button_color', $banner->button_color ?? '#007bff') }}" pattern="^#([A-Fa-f0-9]{6}|[A-Fa-f0-9]{3})$">
							</div>
						</div>
						...
						<div class="mb-3">
							...
							<div class="input-group">
								<input type="color" name="button_text_color" class="form-control form-control-color" value="{{ old('button_text_color', $banner->button_text_color ?? '#ffffff') }}">
								<input type="text" name="button_text_color" class="form-control" value="{{ old('button_text_color', $banner->button_text_color ?? '#ffffff') }}" pattern="^#([A-Fa-f0-9]{6}|[A-Fa-f0-9]{3})$">
							</div>
						</div>
					</div>
				</div>
          ...
          <div class="card-body">
            <h5 class="mb-3">{{ __('Display Settings') }}</h5>
						<div class="mb-3">
							...
							@php
								$allPageSections = \App\Constants\BannerConstants::getPageSections();
								$homePagePositions = \App\Constants\BannerConstants::getHomePagePositions();
								$selectedPosition = old('position', $banner->position);
								$selectedPages = old('target_pages', $banner->target_pages ?? []);
								$isCustomPosition = !array_key_exists($selectedPosition, $homePagePositions) && !collect($allPageSections)->flatten(1)->has($selectedPosition);
							@endphp
							<select name="position" id="position" class="form-control" {{ !$isCustomPosition ? 'required' : '' }}>
								@if(empty($selectedPages) || in_array('home', $selectedPages))
								...
								@foreach($allPageSections as $page => $sections)
									@if($page !== 'home' && (empty($selectedPages) || in_array($page, $selectedPages)))
								...
								@endforeach
								@if($isCustomPosition)
									<option value="{{ $selectedPosition }}" selected>{{ $selectedPosition }} ({{ __('Custom') }})</option>
								@endif
							</select>
							<small class="text-muted">{{ __('Select where this banner/ad should appear. Positions will update based on selected target pages.') }}</small>
							<div class="mt-2">
								<label class="form-check-label">
									<input type="checkbox" id="custom_position_toggle" class="form-check-input" {{ $isCustomPosition ? 'checked' : '' }}>
				...
				<div class="card">
					<div class="card-body">
						...
						<div class="mb-3">
							<label class="form-label">{{ __('Target Pages') }}</label>
							<small class="text-muted d-block mb-2">{{ __('Select specific pages where this banner should appear. Leave empty if "Show on all pages" is checked.') }}</small>

							@php
								$targetPages = \App\Constants\BannerConstants::getTargetPagesGrouped();
								$selectedPages = old('target_pages', $banner->target_pages ?? []);
							@endphp
							...
							<div class="border rounded p-3" style="max-height: 300px; overflow-y: auto;">
								@foreach($targetPages as $categoryKey => $category)
									<div class="mb-3">
										<strong class="d-block mb-2">{{ $category['label'] }}</strong>
							...
							<div class="mt-2">
								<button type="button" class="btn btn-sm btn-outline-primary" id="select-all-pages">{{ __('Select All') }}</button>
								<button type="button" class="btn btn-sm btn-outline-secondary" id="deselect-all-pages">{{ __('Deselect All') }}</button>
							</div>
						</div>
					</div>
				</div>
					...
					<div class="card-body">
						...
						<div id="banner-preview" class="banner-preview-container"
							...
							<div id="preview-image-container"
								...
								<img id="preview-image" src="{{ $banner->image ?? '' }}" alt=""
								...
								<!-- Overlay Content -->
								<div id="preview-overlay-content" style="position: absolute; width: 100%; height: 100%; top: 0; left: 0; pointer-events: none;">
									<div id="preview-text-container" style="position: absolute; padding: 15px; max-width: 80%;">
										<h4 id="preview-title"
									...
									<div id="preview-button-container" style="position: absolute; padding: 15px;">
										<a id="preview-link-button" href="{{ $banner->link_url ?? '#' }}"
...
$(document).ready(function() {
	...
	function getPositionStyles(position) {
		const positions = {
			...
			'bottom-right': { bottom: '10%', right: '5%', left: 'auto', top: 'auto', transform: 'translate(0, 0)' }
		};
	...
	function updatePreview() {
		...
		if (description) {
			$('#preview-description').text(description).show().css('color', textColor);
		} else {
		...
		if (content && content.trim() !== '' && content.trim() !== '<p><br></p>') {
			$('#preview-html-content').html(content).show().css('color', textColor);
		} else {
		...
		if (buttonPosition === 'custom') {
			...
			let customLeft = $('input[name="button_position_custom[left]"]').val();
			buttonPosStyles = {
				top: customTop || '80%',
				left: customLeft || '50%',
				transform: 'translate(-50%, -50%)',
			...
			let textTransform = textPosStyles.transform || 'translate(-50%, -50%)';
			...
			switch (buttonPosition) {
				case 'below-text':
					...
					buttonPosStyles = {
						...
						left: textPosStyles.left || (textLeftPercent + '%'),
						right: textPosStyles.right || 'auto',
	...
	$('input[name="text_position_custom[top]"], input[name="text_position_custom[left]"]').on('input', debouncedUpdatePreview);
	$('input[name="button_position_custom[top]"], input[name="button_position_custom[left]"]').on('input', debouncedUpdatePreview);
	$('input[name="text_color"], input[name="text_background_color"], input[name="text_background_opacity"]').on('input change', updatePreview);
	$('select[name="text_alignment"]').on('change', updatePreview);
	$('input[name="button_color"], input[name="button_text_color"]').on('input change', updatePreview);
	// Image preview
		...
		$.ajax({
			...
			error: function(xhr) {
				if (xhr.status ===
					...
					Swal.fire("{{ __('Validation Error!') }}",errorMsg,
						'error'
					);
				} else {
					Swal.fire("{{ __('Error!') }}",xhr.responseJSON
						?.message ||
	...
	$('#show_on_all_pages').on('change', function() {
		if ($(this).is(':checked')) {
			$('.target-page').prop('disabled', true).prop('checked', false);
		} else {
