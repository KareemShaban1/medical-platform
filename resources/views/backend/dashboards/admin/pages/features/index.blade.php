...
$(document).ready(function() {
    ...
    window.createFeature = function() {
        $.get('{{ route('admin.features.create') }}', function(resp) {
            if (resp.success && resp.html) {
                $('#feature-modal').html(resp.html).modal('show');
            }
        }).fail(function() {
            // Fallback
            $.get('{{ route('admin.features.index') }}?modal=create', function(resp) {
                if (resp.success && resp.html) {
                    $('#feature-modal').html(resp.html).modal('show');
                }
            });
    ...
    window.editFeature = function(id) {
        $.get('{{ route('admin.features.show', ['id' => '__ID__']) }}'.replace('__ID__', id), function(resp) {
            if (resp.success && resp.html) {
                $('#feature-modal').html(resp.html).modal('show');
            }
        });
            ...
            if (result.isConfirmed) {
                $.ajax({
                    ...
                    success: function(resp) {
                        Swal.fire({
                            title: '{{ __('Deleted!') }}',
                            text: resp.message || '{{ __('Feature deleted successfully') }}',
                            icon: 'success',
                    ...
                    error: function(xhr) {
                        const error = xhr.responseJSON?.message || '{{ __('Failed to delete feature') }}';
                        Swal.fire({
                            title: '{{ __('Error') }}',
                            text: error,
                            icon: 'error',
