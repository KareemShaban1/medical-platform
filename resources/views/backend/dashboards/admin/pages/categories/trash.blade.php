    ...
    <div class="row">
        <div class="col-12">
            <div class="page-title-box d-flex justify-content-between align-items-center">
                <h4 class="page-title">{{ __('Trashed Categories') }}</h4>
...
<script>
    $(function() {
        let table = $('#trash-table').DataTable({
            ajax: '{{ route("admin.categories.trash.data") }}',
            columns: [{
                    data: 'id',
                    name: 'id'
                },
        ...
        window.restoreCategory = function(id) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.post('{{ url("admin/categories") }}/' + id + '/restore', {
                        _token: '{{ csrf_token() }}'
                    }, function(response) {
                        table.ajax.reload();
        ...
        window.forceDeleteCategory = function(id) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        url: '{{ url("admin/categories") }}/' + id + '/force-delete',
                        type: 'DELETE',
                        data: {
                            _token: '{{ csrf_token() }}'
                        },
