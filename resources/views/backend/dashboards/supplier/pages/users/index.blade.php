                    ...
                    <div class="col-12">
                              <div class="page-title-box">
                                        <div class="page-title-right">
                                                  <a href="{{ route('supplier.users.trash') }}" class="btn btn-warning me-2">
...
let table = $('#users-table').DataTable({
          ajax: '{{ route("supplier.users.data") }}',
          columns: [{
                              data: 'id',
                              name: 'id'
                    },
...
function loadRoles() {
          $.get('{{ route("supplier.users.roles") }}', function(data) {
                    $('#role').empty();
...
function resetForm() {
          $('#usersForm')[0].reset();
          $('#usersForm').attr('action', '{{ route("supplier.users.store") }}');
          $('#usersId').val('');
...
$('#usersForm').on('submit', function(e) {
          ...
          let id = $('#usersId').val();
          let url = id ?
                    '{{ route("supplier.users.update", ":id") }}'.replace(':id', id) :
                    '{{ route("supplier.users.store") }}';
...
function editUser(id) {
          $.get('{{ route("supplier.users.index") }}/' + id, function(data) {
                    $('#usersId').val(data.id);
                    $('#name').val(data.name);
                    ...
                    // Load roles and select existing one
                    loadRoles();
                    setTimeout(function() {
                    ...
                    $('#usersForm').attr('action',
                              '{{ route("supplier.users.update", ":id") }}'.replace(
                                        ':id', id));
...
function deleteUser(id) {
          Swal.fire({
                    ...
                    if (result.isConfirmed) {
                              $.ajax({
                                        url: '{{ route("supplier.users.destroy", ":id") }}'.replace(':id', id),
                                        method: 'DELETE',
                                        headers: {
...
$(document).on('change', '.toggle-status', function() {
          ...
          $.ajax({
                    url: '{{ route("supplier.users.toggle.status", ":id") }}'.replace(':id', userId),
                    method: 'POST',
                    headers: {
                              'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                    },
