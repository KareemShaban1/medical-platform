			...
			<div class="page-title-box">
				<div class="page-title-right">
					<ol class="breadcrumb m-0">
						...
						<li class="breadcrumb-item active">{{ __('Edit Offer') }}
						</li>
					</ol>
				</div>
			...
			<div class="card">
				<div class="card-body">
					...
					<div class="alert alert-warning">
						...
						<strong>{{ __('Note:') }}</strong> {{ __('This offer is') }}
						<strong>{{ ucfirst($offer->status) }}</strong>.
						{{ __('You can only view the details.') }}
					</div>
					@endif
					...
					<form id="editOfferForm">
						...
						<div class="row">
							...
							<div class="col-md-4">
								<div class="mb-3">
									...
									<small
										class="form-text text-muted">{{ __('Optional discount amount') }}</small>
									<div class="invalid-feedback">
									</div>
								</div>
							</div>
						...
						<div class="row">
							<div class="col-md-6">
								<div class="mb-3">
									<label for="delivery_time"
										class="form-label">{{ __('Delivery Date') }}
										<span
											class="text-danger">*</span></label>
									<input type="date"
										class="form-control"
										...
										required
										value="{{ $offer->delivery_time->format('Y-m-d') }}"
										{{ $offer->status !== 'pending' ? 'readonly' : '' }}>
									<small
										class="form-text text-muted">{{ __('When can you deliver this order?') }}</small>
									<div class="invalid-feedback">
									</div>
								</div>
							</div>
							<div class="col-md-6">
								<div class="mb-3">
									<label
										class="form-label">{{ __('Final Price') }}</label>
									<div
										class="form-control-plaintext">
										<strong
											id="final-price">${{ number_format($offer->final_price, 2) }}</strong>
										<small
											class="text-muted d-block">{{ __('Price - Discount') }}</small>
									</div>
								</div>
							</div>
						</div>
						...
						<div class="row">
							<div class="col-md-6">
								<div class="mb-3">
									...
									<div
										...
										@else
										<span
											class="badge bg-danger">{{ __('Declined') }}</span>
										@endif
									</div>
								</div>
							</div>
							<div class="col-md-6">
								<div class="mb-3">
									<label
										class="form-label">{{ __('Submitted') }}</label>
									<div
										class="form-control-plaintext">
						...
						<div class="mb-3">
							<label for="terms"
								class="form-label">{{ __('Terms & Conditions') }}
								<span
									class="text-danger">*</span></label>
							<textarea class="form-control" id="terms"
								name="terms" rows="4" required
								{{ $offer->status !== 'pending' ? 'readonly' : '' }}>{{ $offer->terms }}</textarea>
							<small
								class="form-text text-muted">{{ __('Minimum 10 characters, maximum 1000 characters') }}</small>
						...
						<div class="alert alert-info">
							...
							{{ __('You can update your offer while it\'s still pending. Once accepted or declined, no changes are allowed.') }}
						</div>
						@elseif($offer->status === 'accepted')
						<div class="alert alert-success">
							<i class="mdi mdi-check-circle me-2"></i>
							<strong>{{ __('Congratulations!') }}</strong>
							{{ __('Your offer has been accepted. Please prepare to fulfill the order according to your terms.') }}
						</div>
						@else
						<div class="alert alert-secondary">
							<i class="mdi mdi-information me-2"></i>
							<strong>{{ __('Declined:') }}</strong>
							{{ __('This offer was not selected. Keep looking for new opportunities!') }}
						</div>
						@endif

						<div class="text-end">
							<a href="{{ route('supplier.offers.index') }}"
								class="btn btn-light me-2">{{ __('Back to My Offers') }}</a>
							<a href="{{ route('supplier.offers.show', $offer->id) }}"
								class="btn btn-info me-2">{{ __('View Details') }}</a>
							@if($offer->status === 'pending')
				...
				<div class="card-body">
					...
					<div class="mb-3">
						...
						<p class="mb-2">
							...
							$offer->id)
							<span
								class="text-success">{{ __('Your offer!') }}</span>
							@else
							<span
					...
					<div class="mb-3">
						...
						<p class="mb-2">
							{{ $offer->updated_at->format('M d, Y H:i') }}</p>
					</div>
				</div>
			</div>
		</div>
...
$(document).ready(function() {
	...
	$('#discount').on('input', function() {
		...
		if (discount > price) {
			...
			$(this).siblings('.invalid-feedback').text(
				'{{ __("Discount cannot exceed the price.") }}'
			);
		} else {
	...
	$('#editOfferForm').on('submit', function(e) {
		...
		submitBtn.prop('disabled', true).html(
			'<i class="mdi mdi-loading mdi-spin me-1"></i> {{ __("Updating...") }}'
		);
				...
				if (response
					...
					Swal.fire({
						...
						.then(() => {
							window.location
								.href =
								"{{ route('supplier.offers.show', $offer->id) }}";
						});
				} else {
					Swal.fire('{{ __("Error!") }}',
						response
