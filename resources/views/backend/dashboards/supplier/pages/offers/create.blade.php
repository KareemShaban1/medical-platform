			...
			<div class="page-title-box">
				<div class="page-title-right">
					<ol class="breadcrumb m-0">
						...
						<li class="breadcrumb-item"><a
								href="{{ route('supplier.available-requests.index') }}">{{ __('Available Requests') }}</a>
						</li>
						<li class="breadcrumb-item active">{{ __('Submit Offer') }}
						</li>
					</ol>
				</div>
				...
				<div class="card-body">
					...
					<form id="createOfferForm">
						...
						<div class="alert alert-info">
							...
							{{ __('Once submitted, you can only edit your offer while it\'s still pending. Make sure all details are accurate.') }}
						</div>

						<div class="text-end">
							<a href="{{ route('supplier.available-requests.show', $request->id) }}"
								class="btn btn-light me-2">{{ __('Cancel') }}</a>
							<button type="submit" class="btn btn-success"
								id="submitBtn">
								<i class="mdi mdi-paper-plane me-1"></i>
								{{ __('Submit Offer') }}
							</button>
						</div>
					</form>
...
$(document).ready(function() {
	...
	function updateFinalPrice() {
		...
		const finalPrice = Math.max(0, price - discount + shipping);
		$('#final-price').text('$' + finalPrice.toFixed(2));
	}

	$('#price, #discount, #shipping').on('input', updateFinalPrice);
	...
	$('#createOfferForm').on('submit', function(e) {
		...
		submitBtn.prop('disabled', true).html(
			'<i class="mdi mdi-loading mdi-spin me-1"></i> {{ __("Submitting...") }}'
		);
