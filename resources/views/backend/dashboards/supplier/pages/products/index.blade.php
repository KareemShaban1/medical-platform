					...
					<div class="row">
						...
						<div class="col-12 col-md-6 mb-3">
							...
							<input type="number" class="form-control"
								step="0.01" id="price_before"
								name="price_before" required>
							<div class="invalid-feedback"></div>
						</div>
						...
						<div class="col-12 col-md-6 mb-3">
							...
							<div class="invalid-feedback"></div>
							<small
								class="text-muted">{{ __('Select multiple images (JPEG, PNG, JPG, GIF, SVG, WEBP)') }}</small>

							<!-- Existing Images Container -->
							<div id="existingImages" class="mt-3"
								style="display: none;">
								<label
									class="form-label">{{ __('Current Images') }}</label>
...
function removeExistingImage(folderId) {
	Swal.fire({
		...
		if (result.isConfirmed) {
			...
			$('#removedImages').append(
				`<input type="hidden" name="removed_images[]" value="${folderId}">`
			);
...
function deleteProduct(id) {
	Swal.fire({
		...
		if (result.isConfirmed) {
			$.ajax({
				url: '{{ route("supplier.products.destroy", ":id") }}'
					.replace(':id', id),
				method: 'DELETE',
				headers: {
					'X-CSRF-TOKEN': $(
...
$(document).on('change', '.toggle-status', function() {
	...
	$.ajax({
		url: '{{ route("supplier.products.toggle.status", ":id") }}'
