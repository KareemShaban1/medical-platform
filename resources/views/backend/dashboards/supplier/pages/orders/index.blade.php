			...
			<div class="page-title-box">
				<div class="page-title-right">
					<div class="d-flex align-items-center gap-2">
						@hasPermission('view orders analytics')
						<a href="{{ route('supplier.orders.analytics') }}"
							class="btn btn-info">
				...
				<div class="modal-body">
					<div class="mb-3">
						...
						<select class="form-select" id="status" name="status"
							...
							<option value="processing">{{ __('Processing') }}
							</option>
							<option value="delivering">{{ __('Delivering') }}
							</option>
							<option value="completed">{{ __('Completed') }}
							</option>
							<option value="cancelled">{{ __('Cancelled') }}
							</option>
						</select>
					</div>
...
<div class="modal fade" id="createRefundModal" tabindex="-1" aria-labelledby="createRefundModalLabel"
	...
	<div class="modal-dialog">
		<div class="modal-content">
			<div class="modal-header">
				<h5 class="modal-title" id="createRefundModalLabel">{{ __('Create Refund') }}
				</h5>
				<button type="button" class="btn-close" data-bs-dismiss="modal"
					aria-label="Close"></button>
			</div>
			<form id="createRefundForm">
				<div class="modal-body">
					<div class="mb-3">
						<label for="refund_type"
							class="form-label">{{ __('Refund Type') }}</label>
						<select class="form-select" id="refund_type"
							name="refund_type" required>
							<option value="partial">{{ __('Partial Refund') }}
							</option>
							<option value="full">{{ __('Full Refund') }}
							</option>
						</select>
					</div>
					<div class="mb-3">
					...
					<div class="mb-3" id="orderItemContainer" style="display: none;">
						<label for="order_item_id"
							class="form-label">{{ __('Select Item (Optional)') }}</label>
						<select class="form-select" id="order_item_id"
							name="order_item_id">
				...
				<div class="modal-footer">
					<button type="button" class="btn btn-secondary"
						data-bs-dismiss="modal">{{ __('Close') }}</button>
					<button type="submit"
						class="btn btn-warning">{{ __('Create Refund') }}</button>
				</div>
			</form>
		</div>
	</div>
			...
			<form id="updatePaymentStatusForm">
				...
				<div class="modal-footer">
					<button type="button" class="btn btn-secondary"
						data-bs-dismiss="modal">{{ __('Close') }}</button>
					<button type="submit"
						class="btn btn-primary">{{ __('Update Payment Status') }}</button>
				</div>
			</form>
		</div>
	</div>
...
$(document).ready(function() {
	...
	window.updateOrderStatus = function(orderId) {
		...
		$.get(`{{ url('supplier/orders') }}/${orderId}/items`, function(
			items) {
			let itemsHtml = '';
