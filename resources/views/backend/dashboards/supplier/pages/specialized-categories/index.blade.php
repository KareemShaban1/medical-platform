		...
		<div class="col-12">
			<div class="card">
				<div class="card-body">
					<div class="row mb-2">
						<div class="col-sm-4">
							@hasPermission('create specialized category')
							<button type="button"
							...
							@hasPermission('attach specialized category')
							<button type="button" class="btn btn-info mb-2"
								data-bs-toggle="modal"
								data-bs-target="#attachCategoryModal">
								<i class="mdi mdi-link me-2"></i>
								{{ __('Join Existing Category') }}
							</button>
							@endhasPermission
						</div>
						<div class="col-sm-8">
						...
						<table id="categories-table"
							...
							<thead class="table-light">
								<tr>
									...
									<th>{{ __('Active Requests') }}
									</th>
									<th>{{ __('Total Suppliers') }}
									</th>
									<th>{{ __('Created') }}</th>
									<th style="width: 85px;">
										{{ __('Action') }}
		...
		<div class="modal-content">
			<div class="modal-header">
				<h5 class="modal-title" id="createCategoryModalLabel">
					{{ __('Create New Specialized Category') }}</h5>
			...
			<form id="createCategoryForm">
				<div class="modal-body">
					<div class="mb-3">
						<label for="name_en"
							class="form-label">{{ __('English Name') }} <span
					...
					<div class="mb-3">
						<label for="name_ar"
							class="form-label">{{ __('Arabic Name') }} <span
								class="text-danger">*</span></label>
				...
				<div class="modal-footer">
					...
					<button type="submit"
						class="btn btn-primary">{{ __('Save Category') }}</button>
				</div>
			</form>
		</div>
	</div>
...
<div class="modal fade" id="attachCategoryModal" tabindex="-1" aria-labelledby="attachCategoryModalLabel"
	aria-hidden="true">
	<div class="modal-dialog">
		<div class="modal-content">
			<div class="modal-header">
				<h5 class="modal-title" id="attachCategoryModalLabel">
					{{ __('Join Existing Category') }}</h5>
				<button type="button" class="btn-close" data-bs-dismiss="modal"
					aria-label="Close"></button>
			</div>
			<form id="attachCategoryForm">
				<div class="modal-body">
					<div class="mb-3">
						<label for="available_category_id"
							class="form-label">{{ __('Available Categories') }}
							<span class="text-danger">*</span></label>
						<select class="form-select" id="available_category_id"
							name="category_id" required>
							<option value="">{{ __('Loading categories...') }}
							</option>
						</select>
						<small
							class="form-text text-muted">{{ __('Select a category that you want to specialize in.') }}</small>
					</div>
				</div>
				<div class="modal-footer">
					<button type="button" class="btn btn-secondary"
						data-bs-dismiss="modal">{{ __('Cancel') }}</button>
					<button type="submit"
						class="btn btn-info">{{ __('Join Category') }}</button>
				</div>
			</form>
		</div>
	</div>
		...
		<div class="modal-content">
			<div class="modal-header">
				<h5 class="modal-title" id="editCategoryModalLabel">
					{{ __('Edit Specialized Category') }}</h5>
				<button type="button" class="btn-close" data-bs-dismiss="modal"
					aria-label="Close"></button>
			</div>
				...
				<div class="modal-body">
					...
					<div class="mb-3">
						<label for="edit_name_ar"
							class="form-label">{{ __('Arabic Name') }} <span
								class="text-danger">*</span></label>
						<input type="text" class="form-control" id="edit_name_ar"
							name="name_ar" required>
					</div>
				</div>
				<div class="modal-footer">
...
$(document).ready(function() {
	...
	$('#editCategoryForm').on('submit', function(e) {
		...
		$.ajax({
			url: "{{ route('supplier.specialized-categories.update', ':id') }}"
				.replace(':id',
					categoryId
				),
			type: 'PUT',
	...
	$.ajax({
		...
		success: function(response) {
			if (response.success) {
				...
				select.html(
					'<option value="">{{ __("Error loading categories") }}</option>'
				);
			}
		},
		error: function(xhr) {
...
function editCategory(id) {
	$.ajax({
		url: "{{ route('supplier.specialized-categories.show', ':id') }}".replace(':id',
			id),
		type: 'GET',
		success: function(response) {
			if (response.success) {
...
function deleteCategory(id) {
	Swal.fire({
		...
		if (result.isConfirmed) {
			$.ajax({
				url: "{{ route('supplier.specialized-categories.destroy', ':id') }}"
					.replace(':id', id),
				type: 'DELETE',
				data: {
					"_token": "{{ csrf_token() }}"
