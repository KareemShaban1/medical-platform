    ...
    <div class="row">
        <div class="col-12">
            <div class="page-title-box d-flex justify-content-between align-items-center">
                ...
                <a href="{{ route('clinic.roles.index') }}" class="btn btn-secondary">
...
<script>
    $(function() {
        let table = $('#trash-table').DataTable({
            ajax: '{{ route("clinic.roles.trash.data") }}',
            columns: [{
                    data: 'id',
                    name: 'id'
                },
        ...
        window.restoreRole = function(id) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.post('{{ url("clinic/roles") }}/' + id + '/restore', {
                        _token: '{{ csrf_token() }}'
                    }, function(response) {
                        table.ajax.reload();
        ...
        window.forceDeleteRole = function(id) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        url: '{{ url("clinic/roles") }}/' + id + '/force-delete',
                        type: 'DELETE',
                        data: {
                            _token: '{{ csrf_token() }}'
                        },
