        ...
        <div class="col-lg-8">
            <div class="card">
                <div class="card-body">
                    ...
                    <div class="row mb-4">
                        <div class="col-md-6">
                            <p class="text-muted mb-1">{{ __('Plan Name') }}</p>
                            <h4 class="mb-0">{{ $subscription->plan->name ?? '-' }}</h4>
                        </div>
                        <div class="col-md-6">
                            <p class="text-muted mb-1">{{ __('Plan Level') }}</p>
                            <h4 class="mb-0">{{ ucfirst($subscription->plan->level ?? '-') }}</h4>
                        </div>
                    </div>

                    <div class="row mb-4">
                        <div class="col-md-6">
                            <p class="text-muted mb-1">{{ __('Start Date') }}</p>
                            <p class="mb-0">{{ $subscription->start_date?->format('M d, Y') ?? '-' }}</p>
                        </div>
                        <div class="col-md-6">
                            <p class="text-muted mb-1">{{ __('End Date') }}</p>
                            <p class="mb-0">
                    ...
                    <div class="list-group list-group-flush">
                        ...
                        <div class="list-group-item px-0">
                            <div class="d-flex justify-content-between align-items-center mb-2">
                                <span class="fw-semibold">{{ $usage->feature->name ?? $usage->feature_code }}</span>
                            ...
                            <small class="text-muted">
                                {{ __('Remaining') }}: {{ max(0, $usage->limit_count - $usage->used_count) }}
                            </small>
                            @endif
                        </div>
                        @endforeach
        ...
        <div class="col-12">
            <div class="card">
                <div class="card-body text-center py-5">
                    ...
                    <p class="text-muted mb-4">{{ __('Subscribe to a plan to unlock all features') }}</p>
                    <a href="{{ route('home') }}#subscriptions-plans" class="btn btn-primary">
...
function cancelSubscription() {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            $.ajax({
                url: '{{ route('clinic.subscriptions.cancel') }}',
                method: 'POST',
                data: {
                    _token: '{{ csrf_token() }}'
                },
