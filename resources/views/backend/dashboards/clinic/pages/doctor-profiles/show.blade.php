		...
		<div class="col-12">
			<div class="page-title-box">
				<div class="page-title-right">
					...
					<a href="{{ route('clinic.doctor-profiles.edit', $profile->id) }}"
	...
	<div class="row">
				...
				<div class="card-body">
					<div class="row">
						<div class="col-lg-3 text-center">
							...
							<h3>{{ $profile->name }}</h3>
							<p class="text-muted lead">{{ $profile->email }}
							</p>

							<div class="mb-3">
								{!! $profile->status_badge !!}
								@if($profile->locked_for_edit &&
								$profile->status === 'approved')
								<br><span class="badge bg-danger mt-1"><i
										class="fa fa-lock"></i>
									{{ __('Locked for Edit') }}</span>
								@endif
							</div>

							@if(in_array($profile->status, ['draft',
							'rejected']))
							<button onclick="submitProfile({{ $profile->id }})"
										...
										<table
											...
											<tbody>
												<tr>
													...
													<td>{{ $profile->phone ?: 'N/A' }}
													</td>
												</tr>
												<tr>
													<th>{{ __('Years of Experience') }}:
													</th>
													<td>{{ $profile->years_experience ?: 'N/A' }}
													</td>
												</tr>
												<tr>
													<th>{{ __('Main Speciality') }}:
													</th>
													<td>{{ $profile->speciality?->name_en ?: 'N/A' }}
													</td>
												</tr>
												<tr>
													<th>{{ __('Status') }}:
													</th>
													<td>{!! $profile->status_badge
														!!}
													</td>
												</tr>
											</tbody>
										...
										<table
											...
											<tbody>
												...
												<tr>
													...
													<td>{{ $profile->reviewed_at->format('Y-m-d H:i') }}
													</td>
												</tr>
												@endif
												@if($profile->reviewer)
												...
												<tr>
													...
													<td>{{ $profile->created_at->format('Y-m-d H:i') }}
													</td>
												</tr>
											</tbody>
										</table>
		...
		<div class="col-lg-6">
			<div class="card">
				...
				<div class="card-body">
					@if($profile->services_offered && count($profile->services_offered) >
					0)
			...
			<div class="card">
				...
				<div class="card-body">
					...
					<div class="mb-3 pb-3 border-bottom">
						...
						@endif
						@if(isset($edu['institution']) && $edu['institution'])
					...
					@endforeach
					@else
					<p class="text-muted">{{ __('No education information provided') }}
					</p>
					@endif
				</div>
			</div>
		...
		<div class="col-lg-6">
			<div class="card">
				...
				<div class="card-body">
					...
					@if(isset($exp['position']) || isset($exp['company']) ||
					isset($exp['start_year']) || isset($exp['end_year']))
					<div class="mb-3 pb-3 border-bottom">
						@if(isset($exp['position']) && $exp['position'])
						<h6 class="mb-1">{{ $exp['position'] }}</h6>
						@endif
						@if(isset($exp['company']) && $exp['company'])
						<p class="text-muted mb-1">{{ $exp['company'] }}</p>
						@endif
						@if(isset($exp['start_year']) || isset($exp['end_year']))
						<small class="text-muted">
							{{ $exp['start_year'] ?? 'Unknown' }} -
					...
					@endforeach
					@else
					<p class="text-muted">{{ __('No experience information provided') }}
					</p>
					@endif
				</div>
			</div>
		...
		<div class="col-lg-6">
			<div class="card">
				...
				<div class="card-body">
					...
					@if(!$profile->linkedin_link && !$profile->twitter_link &&
