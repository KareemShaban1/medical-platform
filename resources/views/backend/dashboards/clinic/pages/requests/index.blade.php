		...
		<div class="col-12">
			<div class="card">
				<div class="card-body">
					<div class="row mb-2">
						<div class="col-sm-4">
							@hasPermission('create purchase request')
...
function deleteRequest(id) {
	Swal.fire({
		...
		if (result.isConfirmed) {
			$.ajax({
				url: "{{ route('clinic.requests.destroy', ':id') }}"
					.replace(':id', id),
				type: 'DELETE',
				data: {
					"_token": "{{ csrf_token() }}"
...
function cancelRequest(id) {
	Swal.fire({
		...
		if (result.isConfirmed) {
			$.ajax({
				url: "{{ route('clinic.requests.cancel', ':id') }}"
					.replace(':id', id),
				type: 'POST',
				data: {
					"_token": "{{ csrf_token() }}"
