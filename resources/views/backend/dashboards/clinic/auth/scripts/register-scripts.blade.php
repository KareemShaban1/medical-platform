...
<script>
    ...
    $(document).ready(function() {
        ...
        function loadCities(governorateId) {
            if (!governorateId) {
                $('#city_id').empty().append("<option value=''>{{ __('Select City ') }}</option>").prop(
                    'disabled',
                    true);
                $('#area_id').empty().append("<option value=''>{{ __('Select Area ') }}</option>").prop(
                    'disabled',
                    true);
                return;
            }
        ...
        $('#governorate_id').on('change', function() {
            ...
            if (governorateId) {
                updateFieldValidation('governorate_id', true);
            } else {
                updateFieldValidation('governorate_id', false,
        ...
        $('#city_id').on('change', function() {
            ...
            // Validate city
            if (cityId) {
                updateFieldValidation('city_id', true);
            } else {
                updateFieldValidation('city_id', false,
                    'City is required');
            }
        ...
        $('#area_id').on('change', function() {
            ...
            // Validate area
            if (areaId) {
                updateFieldValidation('area_id', true);
            } else {
                updateFieldValidation('area_id', false,
                    'Area is required');
            }
        ...
        function validateField(fieldName, value) {
            ...
            // Handle both string and select field values
            const stringValue = typeof value === 'string' ? value.trim() : (value || '');

            // Required check
            if (validator.required && (!value || stringValue === '')) {
                return {
                    valid: false,
        ...
        function clearAllFieldErrors() {
            $('input, textarea, select').each(function() {
                ...
                if (fieldName) {
                    ...
                    $(this).removeClass(
                        'is-valid is-invalid');
                    $(`#${fieldName}_feedback`).removeClass(
                            'valid invalid')
                        .empty();
                }
            });
        ...
        function displayServerErrors(errors) {
            ...
            Object.keys(errors).forEach(function(fieldName) {
                ...
                if (messages && messages.length > 0) {
                    ...
                    serverErrorFields[fieldName] = messages[
                        0];

                    // Show the first error message for each field
                    updateFieldValidation(fieldName, false,
                        messages[0]);
                    ...
                    // Track first visible error field (for focusing)
                    const fieldElement = $(`#${fieldName}`);
                    if (!firstVisibleErrorField && fieldElement.length && fieldElement.is(':visible')) {
                        firstVisibleErrorField = fieldName;
                    }
                }
            });
            ...
            if (firstErrorField) {
                ...
                // Only scroll and focus if there's a visible field with error
                if (firstVisibleErrorField) {
                    const firstVisibleErrorElement = $(`#${firstVisibleErrorField}`);
                    if (firstVisibleErrorElement.length && firstVisibleErrorElement.is(':visible')) {
                        $('html, body').animate({
                            scrollTop: firstVisibleErrorElement
                                .offset().top - 100
                        }, 500);

                        // Focus the field only if it's focusable
                        try {
        ...
        function checkStepValidity(step) {
            let isValid = true;
            const stepFields = step === 1 ? ['clinic_name', 'phone', 'governorate_id',
        ...
        $('input, textarea, select').on('input blur change', function() {
            const fieldName = $(this).attr('id');
            const value = $(this).val();

            // Don't override server errors until user types something new
            if (serverErrorFields[fieldName]) {
                // If user started typing/changing, clear the server error
                if (value && value.length > 0 && $(this).data(
                        'user-typed')) {
        ...
        $('#clinicRegistrationForm').submit(function(e) {
            ...
            submitBtn.prop('disabled', true).html(
                '<i class="fa fa-spinner fa-spin"></i> Sending verification code...'
            );
        ...
        function startOtpTimer() {
            ...
            otpTimer = setInterval(function() {
                ...
                $('#timer').text(String(minutes).padStart(2,
                        '0') + ':' + String(seconds)
                    .padStart(2, '0'));
