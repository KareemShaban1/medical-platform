...
@php
    ...
    if ($buttonPosition === 'custom' && $banner->button_position_custom) {
        ...
        switch($buttonPosition) {
            case 'below-text':
                $buttonPosStyles = ['top' => ($textTop + 20) . '%', 'left' => $textPosStyles['left'] ?? '50%', 'transform' => $textPosStyles['transform'] ?? 'translate(-50%, 0)'];
                break;
            case 'above-text':
                $buttonPosStyles = ['bottom' => (100 - $textTop + 20) . '%', 'left' => $textPosStyles['left'] ?? '50%', 'transform' => $textPosStyles['transform'] ?? 'translate(-50%, 0)'];
                break;
            case 'left-of-text':
                $buttonPosStyles = ['top' => $textPosStyles['top'] ?? '50%', 'left' => '5%', 'transform' => 'translate(0, -50%)'];
                break;
            case 'right-of-text':
                $buttonPosStyles = ['top' => $textPosStyles['top'] ?? '50%', 'right' => '5%', 'left' => 'auto', 'transform' => 'translate(0, -50%)'];
                break;
            default:
...
<div class="banner-container" style="{{ $containerStyle }}">
    <div class="banner-image-container" style="position: relative; width: 100%; height: 100%;">
        ...
        <!-- Overlay Content -->
        <div class="banner-overlay" style="position: absolute; width: 100%; height: 100%; top: 0; left: 0; pointer-events: none;">
            @if($banner->title || $banner->description || $banner->content)
                <div class="banner-text-container" style="{{ $textContainerStyle }}">
                    @if($banner->title)
                        <h4 class="banner-title" style="color: {{ $textColor }}; margin-bottom: 10px; font-weight: bold; text-shadow: 1px 1px 2px rgba(0,0,0,0.5);">
                            {{ $banner->title }}
                        </h4>
                    @endif
                    
                    @if($banner->description)
                        <p class="banner-description" style="color: {{ $textColor }}; margin-bottom: 10px; text-shadow: 1px 1px 2px rgba(0,0,0,0.5);">
                            {{ $banner->description }}
                        </p>
                    @endif
                    
                    @if($banner->content)
                        <div class="banner-content" style="color: {{ $textColor }}; margin-bottom: 10px; text-shadow: 1px 1px 2px rgba(0,0,0,0.5);">
                            {!! $banner->content !!}
                        </div>
                    @endif
                </div>
            ...
            @if($banner->link_url)
                <div class="banner-button-container" style="{{ $buttonContainerStyle }}">
                    <a href="{{ $banner->link_url }}" 
                       ...
                       style="display: inline-block; padding: 8px 20px; background-color: {{ $buttonColor }}; color: {{ $buttonTextColor }}; text-decoration: none; border-radius: 4px; pointer-events: auto; transition: opacity 0.3s;"
                       onmouseover="this.style.opacity='0.9'" 
                       onmouseout="this.style.opacity='1'"
                       onclick="trackBannerClick({{ $banner->id }}, event, '{{ $banner->link_url }}', {{ $banner->open_in_new_tab ? 'true' : 'false' }}); @if(!$banner->open_in_new_tab) return false; @endif">
