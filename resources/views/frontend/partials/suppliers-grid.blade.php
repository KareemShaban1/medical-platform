...
<div class="supplier-card card overflow-hidden"
	data-category="{{ $supplier->category ?? 'equipment' }}"
	data-location="{{ $supplier->location ?? 'local' }}"
	data-rating="{{ $supplier->rating ?? rand(3, 5) }}"
	...
	<div class="p-4">
		...
		<div class="flex items-center mb-3">
			<div class="flex text-yellow-400">
				@for($i = 1; $i <= 5; $i++)
					<i class="fas fa-star {{ $i <= ($supplier->rating ?? 4) ? '' : 'text-gray-300' }}"></i>
					@endfor
			</div>
			<span class="text-sm text-gray-500 ml-2">({{ rand(50, 200) }} reviews)</span>
		</div>
		<div class="flex items-center text-sm text-gray-500 mb-3">
			<i class="fas fa-certificate mr-2"></i>
			<span>{{ $supplier->is_certified ? 'Certified Supplier' : 'Standard Supplier' }}</span>
		</div>
		<div class="flex items-center text-sm text-gray-500 mb-3">
		...
		<div class="flex items-center text-sm text-gray-500 mb-3">
			...
			<span>{{ $supplier->status ? 'Active' : 'Inactive' }}</span>
		</div>
