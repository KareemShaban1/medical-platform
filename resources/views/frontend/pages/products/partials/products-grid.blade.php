...
<div class="product-card bg-white rounded-lg shadow-md overflow-hidden hover:shadow-lg transition-shadow"
	...
	<div class="p-4">
		...
		<div class="flex justify-between items-start flex-col gap-3">
			...
			@endphp
			<span class="text-md text-green-700 font-semibold">{{ number_format($finalPrice, 2) }} {{ __('EGP') }} </span>
			@if($beforeWithFees > $finalPrice)
			<span class="text-sm text-red-500 line-through"> {{ number_format($beforeWithFees, 2) }} {{ __('EGP') }}</span>
			@endif
			</div>
			@if($product->stock > 0)
