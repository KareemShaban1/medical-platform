...
.spinner {
	width: 40px;
	height: 40px;
	border: 4px solid rgba(7, 145, 132, .3);
...
.form-input:focus {
	border-color: #079184;
	box-shadow: 0 0 0 3px rgba(7, 145, 132, .1);
		...
		<div class="max-w-2xl mx-auto animate-fade-in-up animation-delay-400">
			<div class="relative group">
				<input type="text" id="heroSearch"
					placeholder="{{ __('search rental spaces...') }}"
	...
	<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
			...
			<div class="flex items-center space-x-4">
				<div class="bg-gray-100 rounded-xl px-4 py-2">
					<span class="text-gray-600">{{ __('showing') }} <span
							id="resultsCount"
							class="font-bold text-primary">{{ $rentalSpaces->total() }}</span>
						{{ __('spaces') }}</span>
				</div>
				<div class="hidden md:flex items-center space-x-2">
					<span class="text-sm text-gray-500">{{ __('view') }}:</span>
					<button id="gridView"
						class="p-3 bg-gradient-primary text-white rounded-xl">
						<i class="fas fa-th"></i>
					</button>
					<button id="listView"
						class="p-3 bg-gray-200 text-gray-600 rounded-xl">
		...
		<div id="filtersPanel" class="hidden bg-gray-50 rounded-2xl p-6 shadow-lg border border-gray-200">
			<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
				<div class="group">
					<label
						class="block text-sm font-semibold text-gray-700 mb-3">{{ __('search rental spaces') }}</label>
					<div class="relative">
				...
				<div class="group">
					<label
						class="block text-sm font-semibold text-gray-700 mb-3">{{ __('availability') }}</label>
					<div class="relative">
						<select id="availability"
							...
							<option value="monthly">{{ __('monthly') }}
							</option>
						</select>
					</div>
				</div>
	...
	<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
		<div id="loadingSpinner" class="hidden text-center py-12">
			<div class="inline-flex items-center space-x-3">
				...
				<span
					class="text-gray-600 font-medium">{{ __('loading rental spaces...') }}</span>
			</div>
		</div>
		<div id="rentalSpacesGrid"
			class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
			@include('frontend.pages.rental-spaces.partials.rental-spaces-grid', ['rentalSpaces' =>
			$rentalSpaces])
		</div>
		<!-- Enhanced Pagination -->
