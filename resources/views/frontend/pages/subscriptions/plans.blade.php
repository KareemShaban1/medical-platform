            ...
            @if ($plans->count() > 0)
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                    @foreach ($plans as $plan)
                        <div
                            class="plan-card {{ $plan->level === 'advanced' ? 'featured' : '' }} bg-white rounded-2xl p-8 shadow-lg">
                            ...
                            @if (Auth::guard('clinic')->check() || Auth::guard('supplier')->check())
                                ...
                                @endphp
                                @if ($isSubscribed)
                                    <button class="w-full py-3 bg-green-100 text-green-700 rounded-lg font-semibold"
                                        disabled>
                                        <i class="fas fa-check-circle mr-2"></i>{{ __('Current Plan') }}
                                    </button>
                                @else
                                    <button
                                        onclick="subscribeToPlan({{ $plan->id }}, '{{ $plan->level }}', {{ $plan->price }}, this)"
        ...
        <div class="bg-white rounded-2xl shadow-2xl w-full max-w-2xl mx-4">
            ...
            <div class="px-6 py-5 space-y-4">
                ...
                <div id="subscription-discount-section" class="space-y-2">
                    ...
                    <input type="text" id="discount-code" placeholder="{{ __('Enter your code') }}"
            ...
            <div class="flex items-center justify-end gap-3 px-6 py-4 border-t">
                <button type="button" id="subscription-cancel-btn"
                    class="px-4 py-2 rounded-lg border border-gray-300 text-gray-700 hover:bg-gray-50 transition">
                    {{ __('Cancel') }}
                </button>
                <button type="button" id="subscription-confirm-btn"
                    class="px-5 py-2 rounded-lg bg-primary-gradient text-white font-semibold hover:opacity-90 transition">
                    {{ __('Continue') }}
                </button>
            </div>
        </div>
    ...
    <script>
        ...
        function renderPaymentOptions() {
            ...
            availableGateways.forEach((gateway, index) => {
                ...
                const description = gateway.name === 'paymob' ?
                    "{{ __('Pay securely online via Card or Mobile Wallet') }}" :
                    (gateway.name === 'cod' ? "{{ __('Pay when you receive your order') }}" : '');
                const wrapper = document.createElement('label');
                wrapper.className =
                    'flex items-center p-3 border-2 border-gray-300 rounded-lg cursor-pointer hover:border-blue-500 transition';
                wrapper.innerHTML = `
        ...
        function togglePaymobOptions() {
            // No longer needed - payment method selection is handled by Paymob's Unified Checkout
        }
        ...
        function performSubscriptionRequest(planId, btn, paymentGateway = 'cod', discountCode = '') {
            if (btn) {
                btn.disabled = true;
        ...
        document.addEventListener('click', (event) => {
            if (event.target.closest('#subscription-modal-close') || event.target.closest(
            ...
            if (subscriptionState.requiresPayment && availableGateways.length > 0) {
                ...
                closeSubscriptionModal();
                performSubscriptionRequest(subscriptionState.planId, subscriptionState.btn, selectedGateway,
                    discountCode);
                return;
            }
            ...
            closeSubscriptionModal();
            performSubscriptionRequest(subscriptionState.planId, subscriptionState.btn, fallbackGateway,
                discountCode);
        });
    </script>
@endpush
