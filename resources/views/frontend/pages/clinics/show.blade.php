				...
				<div class="clinic-description">
					...
					<div class="doctors-grid"
						...
						<div class="doctor-card"
							style="background: white; border-radius: 12px; padding: 24px; box-shadow: 0 4px 6px rgba(0, 0, 0, 0.05); transition: all 0.3s ease; cursor: pointer; border: 1px solid #e5e7eb;"
							onclick="window.location.href='{{ route('doctors.show', $doctor->id) }}'">
							<div
								style="display: flex; flex-direction: column; align-items: center; text-align: center;">
								@if($doctor->profile_photo_url)
								<img src="{{ $doctor->profile_photo_url }}"
									alt="{{ $doctor->name }}"
									style="width: 100px; height: 100px; border-radius: 50%; object-fit: cover; margin-bottom: 16px; border: 3px solid #10b981;">
								@else
								<div
									style="width: 100px; height: 100px; border-radius: 50%; background: linear-gradient(135deg, #059669, #10b981); display: flex; align-items: center; justify-content: center; margin-bottom: 16px; color: white; font-size: 36px; font-weight: 700;">
									{{ substr($doctor->name, 0, 1) }}
								</div>
								@endif
								<h4
									style="font-size: 18px; font-weight: 600; color: #111827; margin-bottom: 8px;">
									{{ $doctor->name }}
								</h4>
								<p
									style="color: #059669; font-weight: 500; margin-bottom: 12px; font-size: 14px;">
								...
								@if($doctor->bio)
								<p
									style="color: #6b7280; font-size: 14px; line-height: 1.5; margin-bottom: 16px; display: -webkit-box; -webkit-line-clamp: 3; -webkit-box-orient: vertical; overflow: hidden;">
									{{ $doctor->bio }}
								</p>
								@endif
								<button class="btn"
									style="width: 100%; background: #059669; color: white; border: none; padding: 10px 20px; border-radius: 8px; font-weight: 500; transition: all 0.3s ease; cursor: pointer;"
									onmouseover="this.style.background='#10b981'"
									onmouseout="this.style.background='#059669'">
									<i class="fas fa-calendar-plus"
										style="margin-right: 8px;"></i>{{ __('Book Appointment') }}
								</button>
							</div>
						</div>
						@endforeach
					...
					@else
					<p
						style="color: #6b7280; padding: 40px; text-align: center; background: #f8fafc; border-radius: 8px;">
			...
			<div class="clinic-sidebar">
				...
				<div class="sidebar-card">
					...
					<div class="contact-info">
						<div class="contact-item">
							...
							<span
								class="contact-text">{{ $clinic->address }}</span>
						</div>
						<div class="contact-item">
				...
				{{-- <div class="sidebar-card">
					...
					<div class="rating-display">
						...
						<span
							class="rating-value">{{ $clinic->rating ?? 4.5 }}</span>
						<span class="rating-count">({{ rand(50, 200) }}
							reviews)</span>
					</div>
	...
	<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
		...
		<div class="clinics-grid">
			...
			<div class="clinic-card"
				onclick="window.location.href='{{ route('clinics.show', $relatedClinic->id) }}'">
				<h3 class="clinic-card-title">{{ $relatedClinic->name }}</h3>
				<p class="clinic-card-specialization">
					{{ ucfirst($relatedClinic->specialization) }} Clinic</p>
				...
				<p class="clinic-card-description">
					{{ Str::limit($relatedClinic->description ?? 'Professional medical services in a comfortable environment.', 100) }}
				</p>
				<div class="clinic-card-actions">
					<button class="clinic-card-btn">View Details</button>
					<button
						class="clinic-card-btn clinic-card-btn-secondary">Book</button>
				</div>
			</div>
			@endforeach
		</div>
