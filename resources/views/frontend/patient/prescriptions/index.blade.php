...
<div class="min-h-screen bg-gray-50 py-10 px-4">
  <div class="max-w-5xl mx-auto">
    <div class="flex items-center justify-between mb-6">
      <h1 class="text-2xl font-bold text-gray-800">{{ __('My Prescriptions') }}</h1>
    ...
    @if($prescriptions->count())
      <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
        @foreach($prescriptions as $prescription)
          <a href="{{ route('user.prescriptions.show', $prescription->id) }}" class="block bg-white rounded-2xl shadow-xl p-6 hover:shadow-2xl hover:-translate-y-1 transition">
            <div class="flex items-center justify-between">
              ...
              <div class="flex -space-x-2">
                @foreach(array_slice($prescription->images, 0, 3) as $img)
                  <img src="{{ $img }}" class="w-10 h-10 rounded-full border-2 border-white object-cover" alt="prescription image">
                @endforeach
              </div>
            </div>
    ...
    @else
      <div class="text-center bg-white shadow-xl rounded-2xl p-10">
        <i class="fas fa-prescription-bottle-alt text-5xl text-gray-400 mb-3"></i>
        <p class="text-gray-600">{{ __('No prescriptions found') }}</p>
      </div>
    @endif
  </div>
  </div>
