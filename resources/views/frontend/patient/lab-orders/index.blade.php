...
<div class="min-h-screen bg-gray-50 dark:bg-gray-900 py-10 px-4">
    <div class="max-w-4xl mx-auto">
        <div class="bg-white dark:bg-gray-800 shadow-2xl rounded-2xl overflow-hidden">
            ...
            <div class="p-6">
                @if($orders->count() === 0)
                    <p class="text-gray-600 dark:text-gray-300">{{ __('No completed lab results yet.') }}</p>
                @else
                    <ul class="space-y-4">
                            ...
                            <li class="p-4 rounded-xl border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-700">
                                <div class="flex items-center justify-between">
                                    <div>
                                        ...
                                        <p class="text-sm text-gray-500 dark:text-gray-300">{{ __('Clinic') }}: {{ $order->clinic?->name }} • {{ $order->reviewed_at?->format('Y-m-d') }}</p>
                                    </div>
                                    <a href="{{ route('user.lab-orders.show', $order->id) }}" class="px-3 py-2 rounded-lg bg-sky-600 text-white text-sm">
                                        {{ __('View') }}
                                    </a>
                                </div>
                            </li>
