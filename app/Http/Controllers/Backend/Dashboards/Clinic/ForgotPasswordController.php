...
class ForgotPasswordController extends Controller
{
    ...
    {
        return view('backend.dashboards.clinic.auth.forgot-password');
    }
    ...
    public function sendResetOtp(Request $request)
    {
        ...
        try {
            ...
            // Send notification
            $user->notify(new PasswordResetNotification($otp, 'clinic'));
    ...
    {
        $request->validate([
            'user_id' => 'required|exists:clinic_users,id',
            'otp' => 'required|string|size:6'
        ]);

        try {
            $user = ClinicUser::findOrFail($request->user_id);
