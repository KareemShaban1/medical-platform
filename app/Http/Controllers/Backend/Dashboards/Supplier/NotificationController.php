...
class NotificationController extends Controller
{
    ...
    {
        $notifications = auth('supplier')->user()
            ->notifications()
            ->latest()
            ->paginate(20);

        return view('backend.dashboards.supplier.pages.notifications.index', compact('notifications'));
    }
    ...
    public function getLatest()
    {
        ...
        $unreadCount = auth('supplier')->user()->unreadNotifications()->count();
    ...
    {
        $notification = auth('supplier')->user()
            ->notifications()
            ->where('id', $id)
            ->first();
        ...
        if ($notification) {
            ...
            return response()->json([
                'status' => 'success',
                'action_url' => $notification->data['action_url'] ?? route('supplier.dashboard')
            ]);
        }
    ...
    {
        auth('supplier')->user()->unreadNotifications->markAsRead();
