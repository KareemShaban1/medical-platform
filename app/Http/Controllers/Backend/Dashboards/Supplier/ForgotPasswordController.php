...
class ForgotPasswordController extends Controller
{
    ...
    {
        return view('backend.dashboards.supplier.auth.forgot-password');
    }
    ...
    public function sendResetOtp(Request $request)
    {
        ...
        try {
            ...
            // Check if user's supplier is active
            if (!$user->supplier || !$user->supplier->status) {
                return response()->json([
                    'success' => false,
            ...
            // Send notification
            $user->notify(new PasswordResetNotification($otp, 'supplier'));
    ...
    {
        $request->validate([
            'user_id' => 'required|exists:supplier_users,id',
            'otp' => 'required|string|size:6'
        ]);

        try {
            $user = SupplierUser::findOrFail($request->user_id);
    ...
    public function resetPassword(Request $request)
    {
        ...
        try {
            ...
            $user = SupplierUser::findOrFail($userId);
